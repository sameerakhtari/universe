...
export function createSurfaceScene(z=-9730){
  ...
  const clouds=[];const cloudTex=createGlowTexture('#ffffff');for(let i=0;i<28;i+=1){const c=new THREE.Sprite(new THREE.SpriteMaterial({map:cloudTex,transparent:true,opacity:.15+rand(i)*.18,depthWrite:false}));c.position.set((rand(i*3)-.5)*620,75+rand(i*5)*95,-80-rand(i*9)*520);const size=35+rand(i*7)*75;c.scale.set(size*2.2,size*.45,1);c.userData.baseX=c.position.x;c.userData.speed=.07+rand(i*13)*.13;clouds.push(c);group.add(c)}
  ...
  group.userData.update=(time)=>{oceanMat.uniforms.uTime.value=time;clouds.forEach((cloud,i)=>{cloud.position.x=cloud.userData.baseX+Math.sin(time*cloud.userData.speed+i*.8)*24;})};
