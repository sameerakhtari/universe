...
export class UniverseWorld {
  constructor(scene, quality, stages) {
    this.scene=scene;this.quality=quality;this.stages=stages;this.interactives=[];this.updatables=[];this.stageGroups=[];this.starField=null;
  }
  ...
  build() {
    ...
    this.starField=createStarField(this.quality.stars);this.scene.add(this.starField);this.updatables.push(this.starField);
  ...
  _buildObservable() {
    ...
    this._track(g,'observable');
  }

  _buildWeb() {
    const web=createCosmicWeb({count:this.quality.webNodes,span:380,seed:4});web.position.z=-900;this._track(web,'web');
  ...
  _buildEarth(){const em=createEarthSystem({z:-7910,radius:38,withMoon:true,moonDistance:132});this._track(em.group,'earth-moon');this.updatables.push(em.group);em.interactives.forEach(({object,id})=>this._register(object,'earth-moon',id));const earth=createEarthSystem({z:-8330,radius:72,withMoon:false});this._track(earth.group,'earth');this.updatables.push(earth.group);earth.interactives.forEach(({object,id})=>this._register(object,'earth',id));const atmosphereHit=proxy(16);atmosphereHit.position.set(-66,42,-8330);this.scene.add(atmosphereHit);this._register(atmosphereHit,'earth','atmosphere');const orbitHit=proxy(72);orbitHit.position.set(0,0,-8330);this.scene.add(orbitHit);this._register(orbitHit,'orbit','earth-limb')}
