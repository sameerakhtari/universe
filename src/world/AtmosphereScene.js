...
export function createAtmosphereScene(z=-9060){
  ...
  const clouds=[];const cloudTex=createGlowTexture('#ffffff');for(let i=0;i<44;i+=1){const s=new THREE.Sprite(new THREE.SpriteMaterial({map:cloudTex,transparent:true,opacity:.10+rand(i*3)*.16,depthWrite:false}));s.position.set((rand(i*5)-.5)*620,-5+rand(i*7)*75,-80-rand(i*9)*460);const scale=40+rand(i*11)*95;s.scale.set(scale*2.4,scale*.42,1);s.userData.baseX=s.position.x;s.userData.speed=.12+rand(i*13)*.24;clouds.push(s);group.add(s)}
  ...
  group.userData.update=(time)=>{clouds.forEach((cloud,i)=>{cloud.position.x=cloud.userData.baseX+Math.sin(time*cloud.userData.speed+i)*18;cloud.position.z+=Math.sin(time*.08+i)*.006;})};
